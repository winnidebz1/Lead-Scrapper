/**
 * OpenAI GPT Service
 * Uses OpenAI API for business discovery
 */
...
const OPENAI_MODEL = import.meta.env.VITE_OPENAI_MODEL || process.env.OPENAI_MODEL || 'gpt-4o-mini';
...
- directorySource: "AI Discovery (OpenAI)"
...
Return ONLY a valid JSON array of business objects. No markdown, no code blocks, just pure JSON.`;
  ...
  try {
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a business directory researcher. Always return valid JSON arrays only.',
          },
          {
            role: 'user',
            content: prompt,
    ...
    try {
      ...
      // Handle both { businesses: [...] } and [...] formats
    ...
    return businesses.map((item: any) => ({
      ...
      emailSource: item.email
        ...
        : EmailSource.NONE,
      hasWebsite: item.hasWebsite === false ? false : false, // Always false for our use case
      isActive: item.isActive !== false,
      mapsUrl: item.mapsUrl || `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent((item.name || '') + ' ' + city)}`,
      directorySource: item.directorySource || 'AI Discovery (OpenAI)',
